...
// Test resume details endpoint with minimal auth
app.get('/test-auth-resume/:id', async (req, res) => {
  try {
    const resumeId = req.params.id;
    console.log('🧪 Testing with minimal auth for ID:', resumeId);
    
    // Simulate recruiter user
    req.user = { id: 8, role: 'recruiter' };
...
const PORT = 5002;
app.listen(PORT, () => {
  console.log(`🧪 Test server with auth running on port ${PORT}`);
  console.log(`Test URL: http://localhost:${PORT}/test-auth-resume/164`);
});
